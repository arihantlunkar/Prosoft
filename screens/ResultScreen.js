...
import { View, Text, StyleSheet, StatusBar, Linking, FlatList } from "react-native";
...
class ResultScreen extends Component {
  ...
  onShare = async (val) => {
    ...
    const { uri: localUri } = await FileSystem.downloadAsync(url, FileSystem.documentDirectory + relativeFileName).catch((error) => {
      console.error(error);
    });
    await Sharing.shareAsync(localUri).catch((err) => console.log("Sharing::error", err));
            ...
            <Card style={styles.card}>
              <Card.Content>
                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "center" }}>
                  <Chip icon={() => <Icon name="shopping-cart" size={16} color="white" />} onPress={() => {}} textStyle={{ color: "#fff" }} style={{ marginBottom: 20, backgroundColor: "#02b389" }}>
                    Product found : <Text style={{ fontWeight: "bold" }}>{this.state.productName}</Text>
                  </Chip>
                </View>
                <View>
                  <Text>
                    <Text style={{ fontWeight: "bold" }}>Type :</Text> {this.props.route.params.type}
                  </Text>
                  <Text>
                    <Text style={{ fontWeight: "bold" }}>Purpose :</Text> {this.props.route.params.purpose}
                  </Text>
                  <Text>
                    <Text style={{ fontWeight: "bold" }}>Value :</Text> {this.props.route.params.cfm} {this.props.route.params.unit}
                  </Text>
                </View>
              </Card.Content>
            </Card>

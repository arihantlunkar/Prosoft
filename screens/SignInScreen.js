...
class SignInScreen extends Component {
	...
	textInputChange(val) {
		var isValidUser = val.trim().length >= 4;
		this.setState({ username: val });
		this.setState({ check_textInputChange: isValidUser });
		this.setState({ isValidUser: isValidUser });
	}

	handlePasswordChange(val) {
		var isValidPassword = val.trim().length >= 8;
	...
	loginHandle(userName, password) {
		...
		this.props.signIn.signIn(foundUser);
	}
		...
		return (
			<View style={styles.container}>
				...
				<Animatable.View animation="fadeInUpBig" style={[styles.footer, { backgroundColor: theme.colors.background }]}>
					<Text style={[styles.text_footer, { color: theme.colors.text }]}>Username</Text>
...
export default function(props) {
	...
	return <SignInScreen {...props} theme={theme} signIn={signIn} />;
}
